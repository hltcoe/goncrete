...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  Summary summarize(SummarizationRequest query)")
...
func main() {
  ...
  client := goncrete.NewSummarizationServiceClientFactory(trans, protocolFactory)
  ...
  case "summarize":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "Summarize requires 1 args")
      flag.Usage()
    }
    arg9 := flag.Arg(1)
    ...
    argvalue0 := goncrete.NewSummarizationRequest()
    ...
    value0 := argvalue0
    fmt.Print(client.Summarize(value0))
    fmt.Print("\n")
    break
